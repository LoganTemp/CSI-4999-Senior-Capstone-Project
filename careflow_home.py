...
# ---------------- BILLING MENU ----------------
class BillingMenuPage(tk.Frame):
    def __init__(self, parent, controller: CareFlowApp):
        super().__init__(parent, bg=BG_COLOR)
        ...
        tk.Label(content, text="Billing", font=FONT_LARGE, bg=BG_COLOR, fg=FG_COLOR).pack(pady=(10, 10))
        tk.Label(content, text="Please choose an option:", font=FONT_SMALL, bg=BG_COLOR, fg="gray").pack(pady=(0, 24))
 
        tk.Button(
            content, text="Patient Billing",
            font=FONT_MEDIUM, bg="#00897B", fg="white",
            width=18, height=2, relief="flat",
            command=lambda: controller.show_frame("PatientBillingPage")
        ).pack(pady=10)
 
        tk.Button(
            content, text="Staff Billing",
            font=FONT_MEDIUM, bg="#00897B", fg="white",
            width=18, height=2, relief="flat",
...
class BillingPage(tk.Frame):
    def __init__(self, parent, controller: CareFlowApp):
        ...
        self.billing_frame = StaffBillingFrame(self)
        self.billing_frame.pack(fill="both", expand=True)
 
    def on_show(self):
        if hasattr(self, "billing_frame"):
            if getattr(self.billing_frame, "logged_in_staff_id", None):
                if hasattr(self.billing_frame, "_load_recent_bills"):
                    self.billing_frame._load_recent_bills()
 
 
# ---------------- PATIENT BILLING PAGE ----------------
class PatientBillingPage(tk.Frame):
    def __init__(self, parent, controller: CareFlowApp):
        super().__init__(parent, bg=BG_COLOR)
        self.controller = controller
 
        self.patient_billing_frame = PatientBillingFrame(self, back_command=lambda: controller.show_frame("BillingMenuPage"))
        self.patient_billing_frame.pack(fill="both", expand=True)
 
    def on_show(self):
        if hasattr(self, "patient_billing_frame"):
            if getattr(self.patient_billing_frame, "logged_in_patient_id", None):
                if hasattr(self.patient_billing_frame, "refresh"):
                    self.patient_billing_frame.refresh()
